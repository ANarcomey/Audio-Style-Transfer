...
class shallowModel2:
    ...
    def initializeWeights(self):
        ...
        std = np.sqrt(2) * np.sqrt(2.0 / ((self.N_BINS + self.N_FILTERS) * filterSize))
        W_init = np.random.randn(filterSize, filterSize, 1, self.N_FILTERS)*std
    ...
    def getCostTensor(self, W, OUT):
        ...
        Z_OUT = tf.nn.conv2d(OUT,W, strides = [1,2,2,1], padding = 'VALID')
        ...
        ## from coursersa:
        m, n_H, n_W, n_C = A_OUT.get_shape().as_list()
    
        # Reshape the images to have them of shape (n_C, n_H*n_W) (≈2 lines)
        A_s = tf.reshape(tf.transpose(A_style, perm = [3,2,1,0]), [n_C, n_H*n_W])
        A_o = tf.reshape(tf.transpose(A_OUT, perm = [3,2,1,0]), [n_C, n_H*n_W])
        ## end from coursera

        Gs = tf.matmul(A_s, A_s, transpose_b = True)
        Go = tf.matmul(A_o, A_o, transpose_b = True)
